...
export const verifyDatabaseSetup = async () => {
  ...
  try {
    ...
    if (tablesError) {
      console.log("⚠️ Information schema access restricted, checking individual tables...");
      ...
      if (existingTables.length === 3) {
        ...
        return { success: true, tablesExist: 3, message: "All core tables accessible" };
      } else {
    ...
    console.log("✅ Information schema accessible, tables found:", tables?.length);
    return { 
      success: true, 
...
export const runDatabaseHealthCheck = async () => {
  ...
  const isHealthy = results.setup.success && 
...
export const setupDemoData = async () => {
  ...
  try {
    ...
    demoTemplateData.forEach((template, index) => {
      ...
      localStorage.setItem(`demo_template_${demoTemplate.id}`, JSON.stringify(demoTemplate));
    });
