...
export const saveTemplateWithAuth = async (templateData: any, config: any) => {
  ...
  try {
    ...
    localStorage.setItem(`custom_template_${fallbackTemplate.id}`, JSON.stringify(fallbackTemplate));
    ...
    return { success: true, data: fallbackTemplate, isLocal: true, error: error.message };
  }
};
